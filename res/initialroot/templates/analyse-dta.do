...
/*
    local bin = int(min(sqrt(`nobs'),30))
    local h = (`max'-`min')/`bin'
    tempvar hist
    gen `hist' = (round((`var' - `min' - `h'/2 ) / `h')*`h') + `min' + `h'/2
    di `bin' "  " `h' " " `min' " " `max'
    qui tab `hist', matcell(histh) matrow(histw)
    forvalues i=1/`bin' {
        if ( `i' == 1 ) {
            noi di "@histh " _continue
        }
        noi di histh[`i',1] _continue
        if ( `i' == `bin' ) {
            noi di ""
        }
        else {
